...
# Generate Tue/Thu dates from Apr 14 (first Tuesday) through Jun 5
dates <- seq(as.Date("2025-04-15"), as.Date("2025-06-05"), by = "day")
...
schedule <- data.frame(
  ...
  Type = ifelse(
    class_dates %in% as.Date(c("2025-05-12", "2025-05-19")),
    "Evening", "Regular"
  ),
  Topic = ""
)

# Add evening rows (May 12 and May 19 are Mondays — insert after May 8 and May 15)
evening_rows <- data.frame(
  Lecture = NA,
  Date = format(as.Date(c("2025-05-12", "2025-05-19")), "%a, %b %e"),
  Type = "Evening",
  Topic = ""
)

# Insert after lecture on May 8 and May 15
idx_may8  <- which(class_dates == as.Date("2025-05-08"))
idx_may15 <- which(class_dates == as.Date("2025-05-15"))

schedule <- bind_rows(
  schedule[1:idx_may8, ],
  evening_rows[1, ],
  schedule[(idx_may8 + 1):idx_may15, ],
  evening_rows[2, ],
  schedule[(idx_may15 + 1):nrow(schedule), ]
) |>
