...
Some other things you can put in YAML header - author - date - resources (e.g. images, css files) - theme (for presentations)
...
Some themes for presentaion inlucde - solarized - metropolis - night - sky

## Yaml Execute

The `execute` section of the YAML header sets how code chunks are executed. For example, - `echo: false`. - `warning: false` - `message: false`.

You can overrides in an R code chunk using
...
## Lets look at the code in *hello.qmd* and see how it works {.scrollable}
...
-   change to a presentation by replacing html in YAML header "format: revealjs: theme: solarized"
...
-   start with a setup R chunk at the top to load all packages and set global options for your code chunks (e.g. echo, warning, message)
